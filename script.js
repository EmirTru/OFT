...
const initFirebase = async () => {
    try {
        ...
        showModal("Firebase'e bağlanırken bir hata oluştu.", 'error');
    }
    renderUI();
};
...
const setupFirestoreListeners = () => {
    if (!connectionCode || !userId) return;

    // Medya Galerisini Dinleme
...
const createConnection = async () => {
    ...
    try {
        ...
        isVerici = true;
        connectionStatus = `Verici kodu: ${newCode}. Alıcı cihaz bekleniyor...`;
        unsubscribeFunctions = setupFirestoreListeners();
    } catch (e) {
        console.error("Firestore bağlantı oluşturma hatası:", e);
        showModal('Bağlantı oluşturulamadı.', 'error');
    } finally {
        isConnecting = false;
        renderUI();
    }
...
const joinConnection = async () => {
    ...
    try {
        ...
        showModal('Bağlantı hatası.', 'error');
    } finally {
        isConnecting = false;
        renderUI();
    }
...
const startCamera = async () => {
    try {
        ...
        showModal("Kamera erişimi engellendi. Lütfen tarayıcı ayarlarından izin verin.", 'error');
    }
};
...
// UI'yı oluşturan ve güncelleyen ana fonksiyon
const renderUI = () => {
    appRoot.innerHTML = '';
    ...
    let mainContentHTML = `
        ...
        <div class="flex items-center justify-center p-3 rounded-lg bg-gray-700 text-sm font-medium">
            ${isConnecting || isSending ? '<span class="animate-pulse">İşlem yapılıyor...</span>' : `<span>${connectionStatus}</span>`}
        </div>
    `;
        ...
        mainContentHTML += `
            ...
            <div class="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6 p-8">
                <div class="flex-1 flex flex-col items-center justify-center p-6 bg-gray-700 rounded-lg shadow-inner space-y-4">
                    <h2 class="text-xl md:text-2xl font-semibold text-center">Verici (Ekran)</h2>
                    <p class="text-center text-gray-400">
                        Bağlantı kodu oluşturarak diğer cihazdan fotoğraf/video al.
                    </p>
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-6 bg-gray-700 rounded-lg shadow-inner space-y-4">
                    ...
                    <p class="text-center text-gray-400">
                        Verici cihazdan gelen kodu girerek kamerayı aç ve medya gönder.
                    </p>
                ...
                <div class="w-full md:w-1/2 flex flex-col space-y-4">
                    <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
                        ...
                        <button id="record-video-btn" class="${cn('flex-1 flex items-center justify-center p-4 rounded-lg text-white font-semibold transition-all shadow-md disabled:bg-gray-500', isRecording ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700')}" ${isSending ? 'disabled' : ''}>
                            ...
                            ${isRecording ? 'Kaydı Durdur' : 'Video Çek'}
                        </button>
                    </div>
                    <div class="flex-1 mt-6 p-4 rounded-xl bg-gray-700 shadow-inner overflow-hidden flex items-center justify-center text-gray-400">
                        Kamera modundasınız. Çektiğiniz medya diğer cihaza gönderilecek.
                    </div>
                </div>
            `;
        } else {
    ...
    // video ve canvas DOM'a ekle (hidden olarak)
    ...
    // Kamera akışı için video elementini bulup atama
