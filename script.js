...
// ######################################################################################
// Bu kodun düzgün çalışması için, aşağıdaki kuralların Firebase Firestore
// konsolunda "Kurallar" bölümüne yapıştırılması ve yayımlanması GEREKMEKTEDİR.
// Ayrıca, Firebase Authentication'da Anonim (Anonymous) oturum açma yöntemi
...
//     // Bu kural, herhangi bir anonim kullanıcının (giriş yapmış kullanıcının)
//     // yeni bir bağlantı kodu oluşturmasına ve bu kodu okumasına izin verir.
//     // Bu, `createConnection` fonksiyonumuz için gereklidir.
//     match /artifacts/{appId}/public/data/connections/{connectionCode} {
//       allow read, create: if request.auth != null;
//     }
//
//     // Bu kural, bağlantı altında bulunan `media` ve `chat` gibi alt koleksiyonlara
//     // veri eklenmesine ve okunmasına izin verir.
//     // `addDoc` ile fotoğraf, video ve mesaj gönderdiğimiz yer burasıdır.
...
// Bu uygulama, Firebase başlatma için kanvas ortamı tarafından sağlanan
// __firebase_config ve __initial_auth_token global değişkenlerini kullanır.
// Bu değişkenleri değiştirmemek önemlidir.
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : null;
const initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;

// Firestore yolları için uygulama kimliği
...
// Yardımcı fonksiyon: Tailwind sınıflarını birleştirir
...
const setupFirestoreListeners = () => {
    ...
    console.log(`Firestore dinleyicileri kuruluyor. Bağlantı kodu: ${connectionCode}`);

    // Medya Galerisi için dinleyici
    ...
    // Sohbet mesajları için dinleyici
...
const createConnection = async () => {
    ...
    console.log(`Yeni bağlantı oluşturuluyor. Yol: /artifacts/${appId}/public/data/connections/${newCode}`);
    ...
    try {
        ...
        showModal('Bağlantı oluşturulamadı. Lütfen Firebase güvenlik kurallarınızı ve anonim oturum açma ayarını kontrol edin.', 'error');
    } finally {
        isConnecting = false;
        renderUI();
    }
...
const joinConnection = async () => {
    ...
    console.log(`Bağlantıya katılınıyor. Kod: ${mediaCode}`);
    ...
    try {
        ...
        showModal('Bağlantı hatası. Lütfen kodu ve Firebase güvenlik kurallarınızı kontrol edin.', 'error');
    } finally {
        isConnecting = false;
        renderUI();
    }
...
const startCamera = async () => {
    try {
        ...
        connectionStatus = "Kamera erişimi başarısız. Lütfen izinleri kontrol edin.";
        showModal("Kamera erişimi reddedildi. Lütfen tarayıcı ayarlarınızdan izin verin.", 'error');
    }
};

// Bir fotoğraf çeker ve Firestore'a gönderir
...
// Ana fonksiyon: Arayüzü oluşturur ve günceller
const renderUI = () => {
    appRoot.innerHTML = '';
              ...
              <div class="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6 p-8">
                  <div class="flex-1 flex flex-col items-center justify-center p-6 bg-gray-700 rounded-lg shadow-inner space-y-4">
                      ...
                      <p class="text-center text-gray-400">
                          Diğer cihazdan fotoğraf/video almak için bir bağlantı kodu oluşturun.
                      </p>
                  ...
                  <div class="flex-1 flex flex-col items-center justify-center p-6 bg-gray-700 rounded-lg shadow-inner space-y-4">
                      ...
                      <p class="text-center text-gray-400">
                          Bağlanmak ve medya göndermek için gönderici cihazdan gelen kodu girin.
                      </p>
...
const initFirebase = async () => {
    try {
        if (!firebaseConfig) {
            showModal("Firebase yapılandırması bulunamadı. Lütfen Canvas ortamını kontrol edin.", 'error');
        ...
        // Kimlik doğrulama durumu çözülene kadar bekle
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                try {
                    // Eğer özel bir token varsa onu kullan, yoksa anonim giriş yap
                    if (initialAuthToken) {
                    ...
                    console.error("Firebase Auth hatası:", e);
                    let errorMessage = `Firebase bağlantısı kurulurken bir hata oluştu: ${e.message}`;
        ...
        showModal("Uygulama başlatılırken bir hata oluştu: Firebase yapılandırması yanlış olabilir.", 'error');
        isFirebaseReady = false;
        renderUI();
    }
};
